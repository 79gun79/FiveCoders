...
export default function Sidebar() {
  ...
  return (
    <aside className="w-[280px] h-screen border-r border-[#BDBDBD] bg-white flex flex-col">
      <nav>
        <ul className="p-3">
          <li className="flex items-center px-6 py-3 rounded-xl hover:bg-[#EEEEEE] cursor-pointer">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="flex items-center px-8 mt-[10px]">
          <span className="flex-1 h-px bg-[#BDBDBD]" />
        </div>
        <h2 className="text-[16px] text-[#585858] px-8 py-2 mt-[25px]">
          즐겨찾는 커뮤니티
        </h2>
        <ul className="p-2.5">
          {channelItem.map((item) => (
            <li
              key={item.id}
              className="flex items-center text-[16px] px-5.5 py-2.5 rounded-xl hover:bg-[#EEEEEE] cursor-pointer"
            >
              ...
              <button onClick={() => toggleSubscride(item.id)}>
                <TiStarFullOutline
                  className={`text-[20px] transition-colors ${
                    item.isSubscribe
                      ? "text-[#397C79] hover:text-[#51B8B2]"
                      : "text-[#BDBDBD] hover:text-[#51B8B2]"
                  }`}
                />
              </button>
            </li>
