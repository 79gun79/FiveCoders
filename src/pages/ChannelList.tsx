...
import { getSubscribedChannels, setSubscribedChannels } from '../utils/localSubscribe'
...
export default function ChannelList() {
  ...
  const handleDeleteConfirmed = async () => {
    ...
    try {
      ...
      setChannels((prev) => prev.filter((channel) => channel._id !== deleteTargetId))
  ...
  const groupedByGenre = channels.reduce<Record<string, Channel[]>>((acc, channel) => {
  ...
  return (
    <>
                ...
                >
                  {isAdmin && (
                    <button
                      ...
                      className="group absolute right-1 top-1 z-20 flex h-8 w-8 items-center justify-center rounded-full border border-[var(--color-gray3)] bg-[var(--color-bg-white)] shadow-sm transition hover:text-[var(--color-red-caution)] hover:bg-red-70 cursor-pointer"
                    >
                      <FaTrashAlt
      ...
      {createChannelModalOpen && (
        <CreateChannelForm onClose={closeCreateChannelModal} onCreate={handleChannelCreated} />
      )}
          ...
          <>
            ...
            >
              <div className="w-[400px] rounded-[8px] bg-[var(--color-bg-white)] p-8 text-center shadow-lg">
                <p className="mb-[28px] text-[18px] font-medium">채널을 삭제하시겠습니까?</p>
                <div className="flex justify-center gap-4">
                  <button
                    onClick={() => setDeleteConfirmOpen(false)}
                    className="btn-style-modal border border-[var(--color-gray4)] bg-[var(--color-bg-white)] text-[var(--color-text-black)] hover:bg-[var(--color-gray3)] px-4 py-2 rounded"
                  >
                    취소
                  </button>
                  <button
                    onClick={handleDeleteConfirmed}
                    className="btn-style-modal bg-red-600 text-[var(--color-bg-white)] px-4 py-2 rounded hover:bg-red-700"
                  >
                    삭제
                  </button>
                </div>
